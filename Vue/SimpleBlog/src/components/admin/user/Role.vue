  ...
  <div>
    ...
    <el-card style="margin: 18px 2%;width: 95%">
      ...
      <div style="margin: 20px 0 20px 0;float: left">
        ...
        <el-button @click="deleteRoleBatch">批量删除</el-button>
      </div>
    </el-card>
  </div>
</template>
  ...
  methods: {
    ...
    editRole (role) {
      ...
      for (let i = 0; i < role.permissions.length; i++) {
        permIds.push(role.permissions[i].id)
      }
    ...
    deleteRole (role) {
      this.selectedRole = role
      this.$axios.post('/admin/role/delete', {
        id: role.id
      }).then(resp => {
        if (resp && resp.data.responseCode === 200) {
          this.$alert('Deleted!')
    ...
    deleteRoleBatch () {
      var roles = this.$refs.roleTable.selection
      this.$axios.post('/admin/roles/delete', {
        roles
      }).then(resp => {
        if (resp && resp.data.responseCode === 200) {
          this.$alert('Deleted Mult!')
          this.listRoles()
        }
      })
    },
