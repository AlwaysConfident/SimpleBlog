<template>
  <div>
    ...
    <el-link @click="postArticle" :underline="false" target="_blank" class="add-link">
...
export default {
  ...
  methods: {
    loadArticles () {
      ...
      this.$axios.get('/article/' + this.pageSize + '/1').then(resp => {
        if (resp && resp.data.responseCode === 200) {
          _this.articles = resp.data.res.content
          _this.total = resp.data.res.totalElements
        }
      })
    },
    ...
    postArticle () {
      this.$router.push('/blog/article/edit')
    },
    viewArticle (id) {
